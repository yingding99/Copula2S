# Step2 Copula2-Sieve log-likelihood under H0: SNP_1 effect = 0 
...
step2 <- function(para,  x1, x2, bl1, br1, bl2, br2,status1, status2) { 
  
  beta<-c(para[1:(p-1)],0) # set snp = 0
  phi<-para[((p-1)+1):((p-1)+1+m)] 
  alpha<-para[(p-1)+1+m+1]
  kappa<-para[(p-1)+1+m+1+1]
